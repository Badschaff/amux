...
MIXPEEK_REPO="${CLAWDBOT_MIXPEEK_REPO:-$HOME/Dev/mixpeek}"
MANIFEST="$MIXPEEK_REPO/server/infra/gke/staging/15-mvs-stateless-guard-rbac.yaml"
NAMESPACE="mixpeek-staging"

[ -f "$MANIFEST" ] || {
  echo "apply-staging-rbac: manifest not found at $MANIFEST (set CLAWDBOT_MIXPEEK_REPO if the mixpeek checkout lives elsewhere)" >&2
  exit 1
}
