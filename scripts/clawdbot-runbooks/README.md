...
Clawdbot (`scripts/clawdbot-poll.sh`) is a plain script, not an agent. It has
...
## How a card reaches clawdbot

**Only a human (verified local member), never a worker, can route a card to
clawdbot.** Tested live: a worker session gets `cross_board_create_forbidden`
/ `cross_board_reassignment_forbidden` from the board API for both creating
a card with `session=clawdbot` and reassigning an existing card to it — by
...
no agent can quietly hand real infrastructure-executing work to clawdbot on
its own authority. A human does it from the dashboard, or an agent proposes
the exact card (title, `CLAWDBOT_RUNBOOK`, `CLAWDBOT_ARGS`, type `chore`)
for a human to create/reassign.

1. Create or reassign a card to `session=clawdbot`, status `todo`, **type `chore`**
   (clawdbot's claim/close calls ack that type's specific gate criteria;
...
2. Its `desc` names the runbook and its args, one line each, nothing else
   ...
   ```
   CLAWDBOT_RUNBOOK: flip-versioning
   CLAWDBOT_ARGS: bucket=gs://mvs-snapshots
   ```

   Multiple args are comma-separated: `key=val,key2=val2`. No spaces around
   `=` or `,` — the parser is deliberately dumb.

3. `scripts/clawdbot-poll.sh` runs on a timer (see `clawdbot-poll.plist`).
   Each tick: claim the card (`todo -> doing`, the normal lease), run the
   named runbook with `CLAWDBOT_ARG_<KEY>` env vars, then:
...
- `scripts/clawdbot-runbooks/<name>.sh`, executable, `set -euo pipefail`.
- Read args from `CLAWDBOT_ARG_<KEY>` env vars (uppercased, `-` -> `_`).
  **Validate every arg yourself** — clawdbot does not know what a sane
...
- Bounded: clawdbot wraps every run in a timeout (`CLAWDBOT_TIMEOUT_S`,
...
**Credentials are not clawdbot's problem to solve.** Every runbook here
assumes the identity running clawdbot already has whatever `gcloud`/`kubectl`
