#!/usr/bin/env bash
# Clawdbot: the fleet's one shared, non-LLM executor for pre-approved,
# named runbooks. No new board state — it only ever claims cards already
# reassigned to it (`session=clawdbot`, `status=todo`) and drives them
# through the SAME todo -> doing -> done/blocked lifecycle every other
# worker uses. See scripts/clawdbot-runbooks/README.md for the contract.
#
...
# is run a script that already exists in scripts/clawdbot-runbooks/, named
# exactly by the card. Adding a new runbook is a reviewed code change, not
# something any session can trigger at runtime.
#
# Run by com.amux.clawdbot-poll every 5 minutes (see clawdbot-poll.plist).
set -euo pipefail

export PATH="$HOME/.cargo/bin:/usr/local/bin:/usr/bin:/bin"

REPO="${AMUX_REPO:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
RUNBOOK_DIR="${CLAWDBOT_RUNBOOK_DIR:-$REPO/scripts/clawdbot-runbooks}"
LOG="${CLAWDBOT_LOG:-$HOME/.amux/logs/clawdbot.log}"
LOCK="${CLAWDBOT_LOCK:-$HOME/.amux/clawdbot.lock}"
AMUX_URL="${AMUX_URL:-$(amux url 2>/dev/null || echo https://localhost:8824)}"
SESSION="${CLAWDBOT_SESSION_NAME:-clawdbot}"
MAX_PER_TICK="${CLAWDBOT_MAX_PER_TICK:-5}"
CMD_TIMEOUT_S="${CLAWDBOT_TIMEOUT_S:-60}"
...
if ! mkdir "$LOCK" 2>/dev/null; then
  ...
  if [ -n "$owner" ] && kill -0 "$owner" 2>/dev/null; then
    echo "$(date -Iseconds) clawdbot: previous tick still running (pid $owner), skipping"
    exit 0
  fi
  echo "$(date -Iseconds) clawdbot: breaking stale lock (pid ${owner:-unknown} is gone)"
  rm -rf "$LOCK"
  mkdir "$LOCK" 2>/dev/null || { echo "$(date -Iseconds) clawdbot: lost the lock race, skipping"; exit 0; }
fi
LOCK_HELD=1
echo $$ >"$LOCK/pid"

echo "$(date -Iseconds) clawdbot: tick start"

board_json="$(curl -sk --max-time 20 "$AMUX_URL/api/board?all=1&slim=0")"
if [ -z "$board_json" ]; then
  echo "$(date -Iseconds) clawdbot: board unreachable, measured=false, skipping tick"
  exit 0
fi
...
while [ "$i" -lt "$n_considered" ]; do
  ...
  runbook="$(printf '%s\n' "$desc" | sed -n 's/^CLAWDBOT_RUNBOOK: *//p' | head -1 | tr -d '\r')"
  args_line="$(printf '%s\n' "$desc" | sed -n 's/^CLAWDBOT_ARGS: *//p' | head -1 | tr -d '\r')"

  refuse() {
    local reason="$1"
    echo "$(date -Iseconds) clawdbot: refusing $id: $reason"
    n_refused=$((n_refused + 1))
    amux board block "$id" --on "clawdbot refused: $reason" >>"$LOG" 2>&1 || true
  }

  if [ -z "$runbook" ]; then
    refuse "no CLAWDBOT_RUNBOOK: line in desc"
    continue
  fi
  ...
  if [ ! -x "$script" ]; then
    refuse "unknown runbook '$runbook' (no script at scripts/clawdbot-runbooks/$runbook.sh)"
    continue
  fi
  ...
  # (scope) and the card is owned by this session (owner). Clawdbot cards
  # must be created as `type: chore` (see clawdbot-runbooks/README.md); any
  ...
  if ! amux board doing "$id" --checked "Scope is clear" "Has an owner" >>"$LOG" 2>&1; then
    echo "$(date -Iseconds) clawdbot: could not claim $id (already claimed, or not type=chore?), skipping"
    continue
  fi

  # Args become CLAWDBOT_ARG_<KEY> env vars; each runbook validates its own.
  ...
  while [ "$j" -lt "$n_pairs" ]; do
    ...
    arg_env+=("CLAWDBOT_ARG_${key_upper}=${val}")
  done

  echo "$(date -Iseconds) clawdbot: running $runbook for $id (args: $args_line)"
  out_file="$(mktemp -t "clawdbot-$id")"
  ...
  if [ "$exit_code" -eq 0 ]; then
    ...
    # scripts/clawdbot-runbooks/<name>.sh is a real repo path and always
    # matches, so it doubles as the checkable "what actually ran" pointer.
    if printf '%s\n\nrunbook: scripts/clawdbot-runbooks/%s.sh\nargs: %s\nexit: 0\n' "$output" "$runbook" "$args_line" \
        | amux board done "$id" --checked "Outcome recorded in the item (what happened, and why it is closed)" --evidence-stdin >>"$LOG" 2>&1; then
      n_ran=$((n_ran + 1))
      echo "$(date -Iseconds) clawdbot: $id done (runbook $runbook, exit 0)"
    else
      n_refused=$((n_refused + 1))
      printf '%s\n\nrunbook: %s (SUCCEEDED, exit 0) args: %s\n' "$output" "$runbook" "$args_line" \
        | amux board progress "$id" --stdin >>"$LOG" 2>&1 || true
      amux board block "$id" --on "clawdbot: runbook $runbook succeeded but the board refused done (gate/type mismatch) — see progress note" >>"$LOG" 2>&1 || true
      echo "$(date -Iseconds) clawdbot: $id runbook OK but done was REFUSED — left blocked, not silently dropped"
    fi
  else
    n_refused=$((n_refused + 1))
    printf '%s\n\nrunbook: %s\nargs: %s\nexit: %s\n' "$output" "$runbook" "$args_line" "$exit_code" \
      | amux board progress "$id" --stdin >>"$LOG" 2>&1 || true
    amux board block "$id" --on "clawdbot: runbook $runbook exited $exit_code, see log" >>"$LOG" 2>&1 || true
    echo "$(date -Iseconds) clawdbot: $id FAILED (runbook $runbook, exit $exit_code)"
  fi
done

echo "$(date -Iseconds) clawdbot: tick done — considered=$n_considered ran=$n_ran refused=$n_refused"
